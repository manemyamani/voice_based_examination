...
class MultiUserFaceVerification:
    def __init__(self, reference_images_dir: str):
        """
        Initialize the multi-user face verification system
        
        Args:
            reference_images_dir (str): Directory containing reference images
                                      Images should be named as 'user_id.jpg'
        """
    ...
    def verify_single_user(self, duration: int = 25, required_matches: int = 5) -> Dict[str, bool]:
        """
        Verify a single user from webcam feed and stop immediately upon verification
        
        Args:
            duration (int): Maximum duration in seconds for verification (25 seconds)
            required_matches (int): Number of consecutive matches required for verification
            
        Returns:
            Dict[str, bool]: Dictionary mapping user_id to verification status
        """
        verified_users = {}
                        ...
                        if True in matches:
                            ...
                            if face_distances[best_match_index] < 0.4:  # Strict threshold
                                ...
                                if consecutive_matches[user_id] >= required_matches:
                                    ...
                                    html_path = r"C:\face_recogn\login_success.html"
...
def main():
    # Example usage
    reference_dir = r"C:\face_recogn\reference_images"  # Directory containing reference images
    verifier = MultiUserFaceVerification(reference_dir)
